...
function DashboardApp() {
  ...
  const [worldMetadata, setWorldMetadata] = useState<Array<{
    name: string;
    playerCount?: number;
  }> | null>(null);
  const [prefabMetadata, setPrefabMetadata] = useState<
  ...
  const playerRoster = useMemo(() => {
    return Object.values(worldState.players).sort((a: any, b: any) =>
    ...
    return Object.values(worldState.portals)
      ...
      .sort((a: any, b: any) => {
        if (a.status === b.status)
          return (b.lastEnteredAt || "").localeCompare(a.lastEnteredAt || "");
        return a.status === "active" ? -1 : 1;
      });
  }, [worldState.portals]);

  const replayRangeMax = useMemo(
    () => Math.max(replay.events.length - 1, 0),
    [replay.events.length],
  );
  const replayCurrentEvent = useMemo(
    () => replay.events[replay.cursor] || null,
    [replay.events, replay.cursor],
  );
  ...
  const replayEndTime = formatTimestamp(
  ...
  const loadArchives = useCallback(() => {
    if (!isPreviewMode()) {
      api
        ...
        .then((res) => res.data)
        .then((payload) => {
          const list = (payload?.archives || payload || []).sort(
            (a: any, b: any) =>
  ...
  const saveArchive = useCallback(
    (instanceData: any) => {
      const serializable = {
        ...instanceData,
        players: Array.from(instanceData.players || []),
      };
      if (!isPreviewMode()) {
        api
          .post("/archives", {
  ...
  const deleteArchive = useCallback(
    (archiveId: string) => {
      if (!isPreviewMode()) {
        api
          .delete(`/archives/${encodeURIComponent(archiveId)}`)
          .then(() => {
            setSelectedArchive((prev: any | null) =>
              prev?.id === archiveId ? null : prev,
            );
            loadArchives();
          })
          .catch(() => null);
  ...
  const getEventFields = useCallback(
    (ev: VexEvent) => ev.data?.fields || ev.data?.payload || ev.data || {},
    [],
  );
  ...
  const getEventTimestamp = useCallback(
    (ev: VexEvent) =>
      ev.timestamp || ev.data?.timestamp || new Date().toISOString(),
    [],
  );
  ...
  const getPlayerNameFromRef = useCallback(
    (playerRef: any, fallbackId?: string | null) => {
      if (!playerRef) return fallbackId || "Unknown";
      return playerRef.name || playerRef.username || fallbackId || "Unknown";
    },
    [],
  );
  const buildEventFingerprint = useCallback(
    (event: VexEvent) => {
  ...
  const saveEventToLog = useCallback(
    (event: VexEvent) => {
  ...
  const ensurePrefabMetadata = useCallback((prefabPath?: string) => {
    ...
    if (
      prefabMetadataRef.current[prefabPath] ||
      pendingPrefabFetches.current.has(prefabPath)
    )
      return;
    pendingPrefabFetches.current.add(prefabPath);
    api
      .get(`/metadata/prefab/${encodeURIComponent(prefabPath)}`)
      .then((res) => res.data)
      .then((data) => {
        if (!data) return;
        const size =
          data.roomSize || data.size || data.dimensions || data.tiles || data;
        const w = size?.width ?? size?.w ?? size?.x;
        const h = size?.height ?? size?.h ?? size?.z;
        if (Number.isFinite(w) && Number.isFinite(h)) {
          setPrefabMetadata((prev) => ({
            ...prev,
            [prefabPath]: { w: Math.max(1, w), h: Math.max(1, h) },
          }));
        }
      })
      .catch(() => null)
  ...
  const recalculateState = useCallback(
    (eventsOverride?: VexEvent[], skipArchiveSave?: boolean) => {
      ...
      const instanceMetrics: Record<
        string,
        { entityCount: number; killCount: number; roomCount: number }
      > = {};
      const instanceRoomStats: Record<
        string,
        Record<
          string,
          {
        ...
        >
      > = {};
      const instancePlayerStats: Record<
        string,
        Record<string, { kills: number; points: number }>
      > = {};
      ...
      chronological.forEach((ev) => {
        ...
        if (
          (ev.type === "InstanceTeardownStartedEvent" ||
            ev.type === "InstanceTeardownCompletedEvent") &&
          worldName
        ) {
          ensureInstanceState(worldName);
        ...
        if (ev.type === "RoomGeneratedEvent" && worldName && fields.room) {
          ...
          if (!activeInstances[worldName].rooms[roomKey]) {
            ...
            const existingStats = instanceRoomStats[worldName][roomKey] || {
              entities: 0,
              kills: 0,
            };
        ...
        if (
          (ev.type === "WorldEnteredEvent" ||
            ev.type === "PlayerJoinedServerEvent" ||
            ev.type === "InstanceEnteredEvent") &&
          playerRef &&
          worldName
        ) {
          ...
          if (
            players[pid] &&
            players[pid].world &&
            activeInstances[players[pid].world]
          ) {
        ...
        if (ev.type === "PortalEnteredEvent") {
          ...
          if (portalId) {
            ...
            if (playerRef?.name)
              portals[portalId].enteredBy.push(playerRef.name);
        ...
        if (
          ev.type === "EntitySpawnedEvent" ||
          ev.type === "PrefabEntitySpawnedEvent"
        ) {
          entities += 1;
        ...
        if (ev.type === "EntityEliminatedEvent" && worldName) {
          ...
          const killerId = fields.killerId?.toString
            ? fields.killerId.toString()
            : fields.killerId;
          if (killerId) {
            const playerStats = instancePlayerStats[worldName][killerId] || {
              kills: 0,
              points: 0,
            };
            playerStats.kills += 1;
      ...
      Object.keys(instanceMetrics).forEach((worldName) => {
        ...
        activeInstances[worldName].roomStats =
          instanceRoomStats[worldName] || {};
        activeInstances[worldName].playerStats =
          instancePlayerStats[worldName] || {};
      });
      ...
      const activeInstancesCount = Object.values(activeInstances).filter(
        (i: any) => i.active && i.name !== "default",
      ).length;
      const totalInstancesCount = Object.values(activeInstances).filter(
        (i: any) => i.name !== "default",
      ).length;
      const activePortals = Object.values(portals).filter(
        (p: any) => p.status === "active",
      ).length;
      const closedPortals = Object.values(portals).filter(
        (p: any) => p.status === "closed",
      ).length;
      const expiredPortals = Object.values(portals).filter(
        (p: any) => p.status === "expired",
      ).length;
      const totalPortalEntries = Object.values(portals).reduce(
        (sum: number, p: any) => sum + (p.enterCount || 0),
        0,
      );
      const playersInInstances = Object.values(players).filter(
        (p: any) => p.world && p.world.startsWith("instance-"),
      ).length;
      ...
      if (!skipArchiveSave) {
        Object.values(activeInstances).forEach((inst: any) => {
          if (
            !inst.active &&
            !archivedInstancesRef.current.find((a) => a.id === inst.name)
          ) {
            saveArchive(inst);
          }
        });
    ...
    [
      events,
      ensurePrefabMetadata,
      getEventFields,
      getEventTimestamp,
      getEventWorld,
      getPlayerIdFromRef,
      getPlayerNameFromRef,
      saveArchive,
    ],
  );

  const ingestEvent = useCallback(
    (payload: any, options?: { broadcast?: boolean }) => {
      const normalized: VexEvent =
        payload && payload.internalId
          ? payload
          : {
      ...
      if (recentEventFingerprintsRef.current.size > 1000) {
        const entries = Array.from(recentEventFingerprintsRef.current.entries())
          ...
          .slice(0, 300);
        entries.forEach(([key]) =>
          recentEventFingerprintsRef.current.delete(key),
        );
      }
      ...
      const demoData = [
        {
          ...
          data: {
            fields: {
              playerRef: { name: "MBRound18" },
              world: { name: "default" },
            },
          },
        },
        {
          type: "InstanceInitializedEvent",
          data: { fields: { world: { name: "instance-Lava-A" } } },
        },
        {
          type: "PortalCreatedEvent",
          data: {
            fields: {
              world: { name: "default" },
              portalId: "portal-001",
        ...
        try {
          ...
          if (data?.prefabPath && data?.roomSize) {
            ...
            if (Number.isFinite(w) && Number.isFinite(h)) {
              setPrefabMetadata((prev) => ({
                ...prev,
                [data.prefabPath]: { w: Math.max(1, w), h: Math.max(1, h) },
              }));
            }
          }
        } catch {
  ...
  const startReplay = async (instanceName: string) => {
    const stored = await loadEventLog(instanceName);
    const replayEvents =
      stored && stored.length ? stored : buildReplayEvents(instanceName);
    setReplay({
      active: true,
      playing: false,
      cursor: 0,
  ...
  const applyReplayCursor = useCallback(
    (nextCursor: number, sourceEvents?: VexEvent[]) => {
  ...
  const seekReplay = (value: string) => {
    ...
    const clamped = Number.isNaN(next)
      ? 0
      : Math.min(Math.max(next, 0), replayRangeMax);
    setReplay((prev) => ({ ...prev, cursor: clamped }));
    applyReplayCursor(clamped);
  };

  const replayLabel =
    replay.source === "telemetry"
      ? "Telemetry"
  ...
  const normalizeTelemetryEvent = (ev: TelemetryEvent): VexEvent => {
    ...
    const id =
      getTelemetryId(ev) ||
      `${ev.type || ev.data?.type || "event"}-${timestamp}`;
    return {
      internalId: id,
      timestamp,
  ...
  const startTelemetryReplay = useCallback(
    (
      sourceEvents: TelemetryEvent[],
      ...
      let cursor = anchorId
        ? ordered.findIndex((ev) => getTelemetryId(ev) === anchorId)
        : -1;
      if (cursor < 0 && anchor.timestamp) {
        const anchorStamp = toReplayTimestamp(anchor.timestamp);
        cursor = ordered.findIndex(
          (ev) => ev.type === anchor.type && ev.timestamp === anchorStamp,
        );
      }
  ...
  const replayToEvent = useCallback(
    (sourceEvents: TelemetryEvent[], anchor: TelemetryEvent) => {
      startTelemetryReplay(sourceEvents, anchor, false);
    },
    [startTelemetryReplay],
  );

  const replayFromEvent = useCallback(
    (sourceEvents: TelemetryEvent[], anchor: TelemetryEvent) => {
          ...
          <Route
            ...
            element={
              <MapView
                ...
                getRoomSizeForRoom={getRoomSizeForRoom}
              />
            }
          />
          <Route
