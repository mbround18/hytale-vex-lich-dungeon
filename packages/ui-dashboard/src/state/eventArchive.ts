...
export const loadArchivesFromDb = async (): Promise<ArchiveRecord[]> => {
  return withStore('archives', 'readonly', (store) =>
...
export const saveArchiveToDb = async (record: ArchiveRecord) => {
  ...
  await withStore('archives', 'readwrite', (store) =>
...
export const saveEventLogToDb = async (instanceName: string, event: any) => {
  ...
  await withStore('eventLogs', 'readwrite', (store) =>
    new Promise<void>((resolve, reject) => {
      const req = store.get(instanceName);
      req.onsuccess = () => {
        const existing = (req.result as EventLogRecord | undefined) || { id: instanceName, events: [] };
...
export const loadEventLogFromDb = async (instanceName: string): Promise<any[] | null> => {
  return withStore('eventLogs', 'readonly', (store) =>
    new Promise((resolve, reject) => {
      const req = store.get(instanceName);
      req.onsuccess = () => resolve((req.result as EventLogRecord | undefined)?.events || null);
      req.onerror = () => reject(req.error);
    })
  );
};
