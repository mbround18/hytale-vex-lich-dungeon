...
interface Instance {
  ...
  roomStats?: Record<
    string,
    { entities: number; kills: number; order?: number; generatedAt?: string }
  >;
...
export default function MapView({
  ...
  const worldManifest = useMemo(() => {
    ...
    Object.values(worldState.players || {}).forEach((p: Player) => {
      ...
      streamPlayerCounts.set(
        p.world,
        (streamPlayerCounts.get(p.world) || 0) + 1,
      );
    });
  ...
  const layoutData = useMemo(() => {
    ...
    const allArchived = (worldState.archivedInstances || []).map(
      (arch: any) => ({
        ...arch,
    ...
    allArchived.forEach((arch) => {
      if (!rawInstances.find((i) => i.name === arch.name))
        rawInstances.push(arch);
    });
    ...
    const processedInstances = rawInstances.map((inst) => {
      ...
      const maxRoomOrder = Math.max(
        0,
        ...effectiveRooms.map((room) => room.stats?.order || 0),
      );
    ...
    processedInstances.forEach((inst: any) => {
      const neededWidth =
        (currentRow.instances.length ? GAP_TILES : 0) + inst.gridW;
      if (
        currentRow.instances.length > 0 &&
        currentRow.width + neededWidth > MAX_ROW_TILES
      ) {
        rows.push(currentRow);
        currentRow = { instances: [], width: 0, height: 0 };
      }
      inst.renderX =
        currentRow.width + (currentRow.instances.length ? GAP_TILES : 0);
  ...
  const selectedInstance = useMemo(() => {
    ...
    return (
      layoutData.instances.find((inst) => inst.name === selectedWorldId) || null
    );
        ...
        <div className="space-y-1 font-mono text-xs">
          ...
          {room.size && (
            <div className="text-gray-600">
              {room.size.w}x{room.size.h}
            </div>
          )}
          {room.stats?.order && (
            <div className="text-indigo-300">Spawn #{room.stats.order}</div>
          )}
          {(room.stats?.entities || room.stats?.kills) && (
            <div className="text-gray-500">
              Entities: {room.stats?.entities ?? 0} · Kills:{" "}
              {room.stats?.kills ?? 0}
            </div>
          )}
        </div>
  ...
  return (
    <div className="flex w-full h-full bg-[#050608] font-sans overflow-hidden text-gray-200">
      ...
      <div className="w-80 flex flex-col border-r border-white/10 bg-[#0b0d12]/95 backdrop-blur-sm z-20 shadow-2xl">
        <div className="p-4 border-b border-white/10 bg-black/20">
          ...
          <div className="grid grid-cols-2 gap-2 text-[10px] font-mono text-gray-500">
            <div className="flex items-center gap-1.5">
              <Server
                size={12}
                className={serverStats ? "text-emerald-500" : "text-gray-700"}
              />
              <span>
                MEM:{" "}
                {serverStats
                  ? `${toMB(serverStats.system.memory_total - serverStats.system.memory_free)}MB`
                  : "--"}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              <Activity
                size={12}
                className={serverStats ? "text-indigo-500" : "text-gray-700"}
              />
            ...
            <div className="flex items-center gap-1.5">
              <Database
                size={12}
                className={serverStats ? "text-amber-500" : "text-gray-700"}
              />
            ...
            <div className="flex items-center gap-1.5">
              <Users
                size={12}
                className={serverStats ? "text-rose-500" : "text-gray-700"}
              />
        ...
        {selectedInstance && (
          <div className="mx-3 mb-3 rounded-md border border-white/10 bg-black/30 p-3 text-[10px] font-mono text-gray-400">
            <div className="text-gray-500 uppercase tracking-widest mb-2">
              Instance Pulse
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                Rooms:{" "}
                {selectedInstance.stats?.roomCount ??
                  selectedInstance.rooms.length}
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
          ...
          >
            <div
              className={`p-1.5 rounded-md ${selectedWorldId === null ? "bg-indigo-500 text-white" : "bg-gray-800"}`}
            >
              <Globe size={16} />
            </div>
            <div>
          ...
          <div className="px-2 pb-2 text-[10px] font-bold text-gray-600 uppercase tracking-wider font-mono">
            Detected Signals
          </div>
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  ...
                  <div className="min-w-0">
                    <div
                      className={`text-xs font-medium truncate ${world.active ? "text-gray-200" : "text-gray-600"}`}
                    >
                      {world.name}
                    </div>
                    <div className="flex items-center gap-2 text-[10px] font-mono">
                      <span
                        className={
                          world.active ? "text-emerald-500" : "text-gray-600"
                        }
                      >
      ...
      <div className="flex-1 flex flex-col relative bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-900 to-black">
        ...
        <div className="absolute top-0 left-0 right-0 p-4 z-10 flex justify-between items-start pointer-events-none">
          <div>
            ...
            <p className="text-xs text-indigo-400/60 font-mono mt-1">
              {layoutData?.instances.length || 0} SECTORS //{" "}
              {Object.keys(worldState.players || {}).length} ENTITIES // MEMORY:{" "}
              {serverStats
                ? toMB(
                    serverStats.system.memory_total -
                      serverStats.system.memory_free,
                  )
                : "?"}
              MB
            </p>
        ...
        <div
          ref={containerRef}
          className="flex-1 w-full h-full cursor-move relative z-0"
        >
          {!layoutData ? (
            ...
            <svg ref={svgRef} width="100%" height="100%">
              ...
              >
                  ...
                  return (
                    <InstanceGroup
                      ...
                      getRoomStyle={(
                        ...
                        instance: Instance,
                        defaults: any,
                      ) => getStyledRoom(room, instance, defaults)}
                ...
                {Object.values(worldState.players || {}).map((player) => {
                  const inst = layoutData.instances.find(
                    (i) => i.name === player.world,
                  );
                  if (!inst) return null;

                  const [rx, rz] = player.roomKey
                    ? player.roomKey.split(",").map(Number)
                    : [0, 0];
                  ...
                  const px =
                    (inst.renderX + (room.x - inst.minX) + size.w / 2) *
                    TILE_SIZE;
                  const py =
                    (inst.renderY + (room.z - inst.minZ) + size.h / 2) *
                    TILE_SIZE;

                  const playerId =
                    player.uuid || player.playerId || player.id || player.name;
                  const playerStats = playerId
                    ? inst.playerStats?.[playerId]
                    : null;
...
const InstanceGroup = React.memo(
  ({ inst, getRoomSize, onRoomHover, onRoomLeave, getRoomStyle }: any) => {
    ...
    return (
      <g transform={`translate(${pixelX}, ${pixelY})`}>
        ...
        {inst.rooms.map((room: Room) => {
          ...
          const styled =
            typeof getRoomStyle === "function"
              ? getRoomStyle(room, inst, baseStyle)
              : baseStyle;

          return (
            <g
              key={`${room.x},${room.z}`}
              transform={`translate(${rx}, ${rz})`}
            >
              <rect
                width={rw - 2}
                height={rh - 2}
...
const PlayerDot = ({
  ...
  return (
    ...
    >
      ...
      />
      <circle
        r={4 * Math.max(0.6, scale)}
        fill="#f43f5e"
        className="drop-shadow-[0_0_8px_rgba(244,63,94,1)]"
      />
      <text
        y={-12 * scale}
        textAnchor="middle"
...
const LegendItem = ({ color, label, type = "box" }: any) => (
  <div className="flex items-center gap-2">
    {type === "box" ? (
      ...
      <div
        className={`w-2 h-2 rounded-full ${color} shadow-[0_0_5px_currentColor]`}
      />
    )}
    <span className="text-[10px] font-bold uppercase text-indigo-200 tracking-wider">
      {label}
    </span>
  </div>
);
