...
public class DemoHudCommand extends AbstractCommand<Object> {
  ...
  public DemoHudCommand(@Nonnull HytaleLogger logger) {
    super("dhud", "Shows a demo HUD by name or resets HUD", false, (ref, store) -> new Object());
    this.hudArg = Objects.requireNonNull(
        withRequiredArg("hud", "HUD name or reset", ArgTypes.STRING), "hudArg");
  ...
  protected void execute(
    ...
    String hudName = context.get(Objects.requireNonNull(hudArg, "hudArg"));
    ...
    HudFactory factory = huds.get(key);
    if (factory == null) {
      context.sendMessage(
    ...
    CustomUIHud hud = factory.create(context, store, ref, playerRef);
    if (hud == null) {
