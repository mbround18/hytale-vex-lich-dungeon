...
Requirements:
- Dev logs live in static/dev/logs/ and are indexed by static/dev/logs/index.json (most recent first).
...
Steps:
1) Read static/dev/logs/index.json and determine the most recent log date (from filename or frontmatter).
...
5) Write a concise dev log with:
   ...
   - Notes (optional)
6) Update static/dev/logs/index.json to add the new log at the top.
